...
// 根据系统架构返回各项目(官方)发行包的命名
// Xray: Xray-linux-${MACHINE}.zip (参考官方安装脚本的 MACHINE 映射)
// cloudflared: cloudflared-linux-${ARCH}
// nezha-agent: nezha-agent_linux_${ARCH}.zip
...
function getSystemArchitecture() {
  return getDownloadInfo().family;
}

// 从 zip 包中解压出一个文件（只实现当前项目所需的最小能力）
// 1) 优先尝试调用系统 unzip（若存在）
// 2) 若 unzip 不存在，则用纯 JS 解析 zip（支持 store/deflate）
function extractFileFromZip(zipPath, wantedBasename, outPath) {
  // 尝试使用 unzip -p
  try {
    // unzip -p 会把文件内容输出到 stdout
    // 这里用 execSync 直接重定向到目标文件
    execSync(`unzip -p "${zipPath}" "${wantedBasename}" > "${outPath}"`, { stdio: 'ignore' });
    return;
  } catch (_) {
    // fallback to JS
  }
  ...
  let eocdOffset = -1;
  const maxSearch = Math.max(0, buf.length - 22 - 0xFFFF);
  ...
  while (ptr + 46 <= end) {
    if (buf.readUInt32LE(ptr) !== cdhSig) break;
    ...
    if (!entryName.endsWith('/')) {
      ...
      if (base === wantedBasename) {
        target = { entryName, method, compressedSize, uncompressedSize, localHeaderOffset };
        break;
      }
    }
  ...
  if (!target) {
    throw new Error(`File '${wantedBasename}' not found in zip`);
  }

  const lfhSig = 0x04034b50;
  const lfh = target.localHeaderOffset;
  if (buf.readUInt32LE(lfh) !== lfhSig) {
    throw new Error('Invalid zip: local file header signature not found');
  }
  const localNameLen = buf.readUInt16LE(lfh + 26);
  const localExtraLen = buf.readUInt16LE(lfh + 28);
  ...
  // 简单校验
  if (target.uncompressedSize && data.length !== target.uncompressedSize) {
    // 某些 zip 可能会有 0，这里只在非 0 时校验
    throw new Error('Zip extract size mismatch');
  }
...
async function downloadFilesAndRun() {  
  ...
  const downloadFileAsync = (filePath, fileUrl) =>
    new Promise((resolve, reject) => {
      downloadFile(filePath, fileUrl, (err) => {
        if (err) reject(new Error(err));
        else resolve(filePath);
      });
    });

  // 兼容官方发布包(zip)：先下载 zip，再解压出可执行文件
  const downloadAndPrepare = async (fileInfo) => {
    if (!fileInfo.archive) {
      await downloadFileAsync(fileInfo.fileName, fileInfo.fileUrl);
      return fileInfo.fileName;
    }

    const zipPath = `${fileInfo.fileName}.zip`;
    await downloadFileAsync(zipPath, fileInfo.fileUrl);
    extractFileFromZip(zipPath, fileInfo.archive.innerBasename, fileInfo.fileName);
    try { fs.unlinkSync(zipPath); } catch (_) { /* ignore */ }
    return fileInfo.fileName;
  };

  const downloadPromises = filesToDownload.map(downloadAndPrepare);
...
function getFilesForArchitecture(architecture) {
  // 使用官方发布源：
  // - Xray-core: GitHub Releases (zip)
  // - cloudflared: GitHub Releases (binary)
  // - nezha-agent: GitHub Releases (zip)
  const info = getDownloadInfo();

  const xrayUrl = `https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-${info.xrayMachine}.zip`;
  const cloudflaredUrl = `https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${info.cloudflaredArch}`;

  const baseFiles = [
    {
      fileName: webPath,
      fileUrl: xrayUrl,
      archive: { innerBasename: 'xray' }
    },
    {
      fileName: botPath,
      fileUrl: cloudflaredUrl
    }
  ];
